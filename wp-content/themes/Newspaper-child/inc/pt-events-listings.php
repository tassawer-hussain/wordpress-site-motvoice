    ...
    function widget( $args, $instance ) {
		...
		if ( $book_review_query->have_posts() ) {           
			...
			while ( $book_review_query->have_posts() ) {
				...
				echo		'</div>';
				echo	'</div>';
            	echo 	($counter == ($conference_event_count-1)) ? '<a href="'.get_category_link(34).'" class="fundraising-seemore">View More...</a>' : '';
				echo '</div>';
			...
			echo '<div class="conf-box-section">No event found.</div>';
		}
		echo "</div>";
		wp_reset_query(); 
		echo $after_widget;
...
class FundRising_Events extends WP_Widget {
    ...
    function widget( $args, $instance ) {
		...
		// Display widget title
		echo '<div class="conference-calander fundraising-calender"><div class="conf-heading"><h3>';
		echo apply_filters( 'widget_title', $widget_title );
		echo "</h3></div>";
		// Check if any posts were returned by query
		if ( $book_review_query->have_posts() ) {           
			...
			while ( $book_review_query->have_posts() ) {
				...
				echo		'</div>';
				echo	'</div>';
            	echo 	'<a href="'.get_category_link(33).'" class="fundraising-seemore">View More...</a>';
				echo '</div>';
